...
ALTER TABLE ugc_deletions ADD COLUMN deletion_type deletion_type NOT NULL DEFAULT 'normal';
...
ALTER TABLE ugc_deletions ADD COLUMN deleted_by_id INT REFERENCES users(id) ON DELETE SET NULL;

-- Add index for querying by deletion type
CREATE INDEX idx_ugc_deletions_type ON ugc_deletions(deletion_type);

-- Add legal_hold_at timestamp for legal hold records
ALTER TABLE ugc_deletions ADD COLUMN legal_hold_at TIMESTAMP;

-- Add legal_hold_by to track who placed the legal hold
ALTER TABLE ugc_deletions ADD COLUMN legal_hold_by INT REFERENCES users(id) ON DELETE SET NULL;

-- Add legal_hold_reason for legal hold documentation
ALTER TABLE ugc_deletions ADD COLUMN legal_hold_reason TEXT;
